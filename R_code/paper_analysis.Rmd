...
summm<- enriched_test %>% group_by(tree_id) %>% summarise(mean_true = mean(true_binary_support), mean_pred = mean(prob_predictions), mean_bayes = mean(feature_aBayes_iqtree_support), mean_bayes_cal = mean(predictions_feature_aBayes_iqtree_support_only_boot),
                                                mean_alrt = mean(feature_aLRT_iqtree_support), mean_alrt_cal = mean(predictions_feature_aLRT_iqtree_support_only_boot)
                                                
                                                )  

summm%>% ggplot(aes(x=mean_true,y=mean_bayes))+geom_point()+geom_abline(slope=1, intercept = 0)
summm%>% ggplot(aes(x=mean_true,y=mean_alrt))+geom_point()+geom_abline(slope=1, intercept = 0)

summm %>% ggplot(aes(x=mean_true,y=mean_bayes_cal))+geom_point()+geom_abline(slope=1, intercept = 0)
summm %>% ggplot(aes(x=mean_true,y=mean_alrt_cal))+geom_point()+geom_abline(slope=1, intercept = 0)

summm  %>% ggplot(aes(x=mean_true,y=mean_pred))+geom_point()+geom_abline(slope=1, intercept = 0)


pivoted<- summ %>% pivot_longer()
```
...
out_path <-paste(prefix,"ML_results/",program,"/performance_text",".tsv", sep = "")

paper_performance<-final_models_performance  %>% filter (metric_type=='all_data') %>% dplyr::select (dataset,analysis_type,name,mcc_0.5,logloss, AUC,average_precision, tp_0.5, tn_0.5, fp_0.5, fn_0.5)  %>% arrange(dataset) %>% mutate (description = case_when(analysis_type == "full_standard" & name =="model_standard" ~ "all features", 
...
paper_performance
write_tsv(paper_performance,out_path)
...
Variable importance 
```{r}
out_path <-paste(prefix,"ML_results/",program,"/vi_text",".tsv", sep = "")

vi_data<- vi %>% arrange (`...1`) %>% mutate (feature_name = case_when(
  `...1` == "feature_min_ll_diff"~ "Minimum log-likelihood difference between an NNI neighbor near the bipartition and current tree",
  `...1` =="feature_max_ll_diff"~ "Maximum log-likelihood difference between an NNI neighbor near the split and current tree",
...
)) %>% dplyr::select (`...1`,feature_name,`Gini-importance`, mcc ) %>% arrange(feature_name,-`Gini-importance`)
vi_data
...
```
```{r}
test_group_performance %>% filter (grouping_col_name =='feature_msa_n_loci') 
```
