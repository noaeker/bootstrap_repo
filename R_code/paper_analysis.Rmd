...
paper_performance_iqtree<-iqtree_final_models_performance  %>% filter (metric_type=='all_data') %>% dplyr::select (dataset,analysis_type,name,brier_loss,logloss, AUC,average_precision,mcc_0.5, tp_0.5, tn_0.5, fp_0.5, fn_0.5)  %>% arrange(dataset) %>% mutate (description = case_when(analysis_type == "full_standard" & name =="model_standard" ~ "all features", 
                                ...
                                )) %>% mutate(TPR =`tp_0.5`/(`tp_0.5`+`fn_0.5`),TNR =`tn_0.5`/(`tn_0.5`+`fp_0.5`),FPR =`fp_0.5`/(`tn_0.5`+`fp_0.5`),FNR =`fn_0.5`/(`tp_0.5`+`fn_0.5`)) %>% dplyr::select (dataset, description, brier_loss,logloss,AUC,average_precision, mcc_0.5,FPR,FNR, )  %>% arrange(dataset)
