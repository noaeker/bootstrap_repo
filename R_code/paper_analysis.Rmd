...
fasttree_enriched_test = read_tsv(paste(prefix,"ML_results/fasttree/final_model/full_model/test_model_standard_enriched.tsv", sep = ""))
...
iqtree_enriched_test = read_tsv(paste(prefix,"ML_results/iqtree/final_model/full_model/test_model_standard_enriched.tsv", sep = ""))
...
raxml_enriched_test = read_tsv(paste(prefix,"ML_results/raxml/final_model/full_model/test_model_standard_enriched.tsv", sep = ""))
...
paper_performance<-final_models_performance  %>% filter (metric_type=='all_data') %>% dplyr::select (dataset,analysis_type,name,mcc,balanced_accuracy_score,precision, recall, AUC,average_precision)  %>% arrange(dataset) %>% mutate (description = case_when(analysis_type == "full_standard" & name =="model_standard" ~ "all_features", 
                                ...
                                )) %>% dplyr::select (dataset, description, mcc, balanced_accuracy_score,precision, recall)  %>% arrange(dataset)
...
n_loci_error<-test_group_performance %>% filter (grouping_col_name =='feature_msa_n_loci') %>%  mutate(grouping_col = factor(grouping_col, levels = c('(4.999, 69.0]','(69.0, 129.0]','129.0, 236.0]','	(236.0, 827.0]]')))  %>% ggplot(aes(y=grouping_col, x=Balanced_Accuracy))+geom_col()+ expand_limits(y = c(0.5,1))+ylab('Number\nof positions')+xlab('Balanced\naccuracy')+theme(text = element_text(size = 11))
...
summary(enriched_test$feature_msa_n_seq)
```
