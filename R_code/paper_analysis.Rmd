...
```{r}
prefix = "/Users/noa/Workspace/ML_results"
...
General performance
```{r}
final_models_performance  %>% filter (dataset=='test', metric_type=='all_data') %>% dplyr::select (analysis_type,name,balanced_accuracy_score, mcc, AUC)
```
...
Validation_data
```{r}
validation_data %>% dplyr::select (boot_run_time, extraction_running_time,nni_ll_eval_running_time)
